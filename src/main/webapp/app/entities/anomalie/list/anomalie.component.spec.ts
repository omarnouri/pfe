import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { of } from 'rxjs';

import { AnomalieService } from '../service/anomalie.service';

import { AnomalieComponent } from './anomalie.component';

describe('Anomalie Management Component', () => {
  let comp: AnomalieComponent;
  let fixture: ComponentFixture<AnomalieComponent>;
  let service: AnomalieService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      declarations: [AnomalieComponent],
    })
      .overrideTemplate(AnomalieComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(AnomalieComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(AnomalieService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.anomalies?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });
});
