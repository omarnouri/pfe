...
/**
 * REST controller for managing {@link com.sopra.pfe.domain.Chaine}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ChaineResource {
    ...
    public ChaineResource(ChaineRepository chaineRepository) {
     ...
     */
    @PostMapping("/chaines")
    public ResponseEntity<Chaine> createChaine(@Valid @RequestBody Chaine chaine) throws URISyntaxException {
        ...
        Chaine result = chaineRepository.save(chaine);
    ...
    public ResponseEntity<Chaine> updateChaine(
        @PathVariable(value = "id", required = false) final Long id,
        @Valid @RequestBody Chaine chaine
        ...
        if (!chaineRepository.existsById(id)) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, chaine.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/chaines/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Chaine> partialUpdateChaine(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody Chaine chaine
    ) throws URISyntaxException {
        ...
        Optional<Chaine> result = chaineRepository
            .findById(chaine.getId())
            .map(existingChaine -> {
                if (chaine.getLibelle() != null) {
                    existingChaine.setLibelle(chaine.getLibelle());
                }
                if (chaine.getProcess() != null) {
                    existingChaine.setProcess(chaine.getProcess());
                }

                return existingChaine;
            })
            .map(chaineRepository::save);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, chaine.getId().toString())
        );
    }
    ...
    public List<Chaine> getAllChaines() {
        log.debug("REST request to get all Chaines");
    ...
    public ResponseEntity<Chaine> getChaine(@PathVariable Long id) {
        log.debug("REST request to get Chaine : {}", id);
        Optional<Chaine> chaine = chaineRepository.findById(id);
